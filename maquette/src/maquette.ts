...
let updateChildren = function(vnode: VNode, domNode: Node, oldChildren: VNode[] | undefined, newChildren: VNode[] | undefined, projectionOptions: ProjectionOptions) {
  ...
  if (compare) {
    const oldChildrenKeys: Array<string | {}> = [];
    const newChildrenKeys: Array<string | {}> = [];
    for (const child of oldChildren) {
      const key = <string>((child.properties && child.properties.key) || child.vnodeSelector);
      oldChildrenKeys.push(key);
    }
    for (const child of newChildren) {
      const key = <string>((child.properties && child.properties.key) || child.vnodeSelector); 
      newChildrenKeys.push(key);
    }
    const oldChildrenSearch = oldChildrenKeys.slice(0);
    const newChildrenSearch = newChildrenKeys.slice(0);
    const oldChildrenLength = oldChildrenKeys.length;
    const newChildrenLength = newChildrenKeys.length;
    let changes: Array<[ number, { removed: number[]; moved: number[]; added: number[]; } ]> = [];
    let removed: number[] = [];
    let moved: number[] = [];
    let added: number[] = [];
    let oldIndex = 0;
    ...
    for (; oldIndex < oldChildrenLength || newIndex < newChildrenLength; ) {
      let oldKey = oldChildrenKeys[oldIndex];
      let newKey = newChildrenKeys[newIndex];
      if (oldKey === newKey && oldIndex < oldChildrenLength && newIndex < newChildrenLength && oldChildrenSearch[oldIndex] !== used) {
        // these are identical
        textUpdated = updateDom(oldChildren[oldIndex], newChildren[newIndex], projectionOptions) || textUpdated;

        if (removed.length || moved.length || added.length) {
          changes.push([matched + 1, { removed, moved, added }]);
        ...
        matched = oldIndex;
        oldChildrenSearch[oldIndex] = used;
        newChildrenSearch[newIndex] = used;

        ++oldIndex;
        ++newIndex;
      }
      else {
        const oldInNew = (oldChildrenSearch[oldIndex] === used ? -1 : newChildrenSearch.indexOf(oldKey, newIndex));
        const newInOld = oldChildrenSearch.indexOf(newKey, oldIndex);
        if (oldInNew === -1 && newInOld === -1) {
          // no mutual match
          if (oldIndex < oldChildrenLength) {
            const to = newChildrenSearch.indexOf(oldKey);
            if (to === -1) {
              removed.push(oldIndex);
            }
            else {
              moved.push(oldIndex);
              newChildrenSearch[to] = used;
            }
            ...
            for (; newIndex < newChildrenLength; newIndex++) {
              newKey = newChildrenKeys[newIndex];
              const from = oldChildrenSearch.indexOf(newKey);
              if (from !== -1) {
                oldChildrenSearch[from] = used;
              }
              added.push(newIndex);
            }
          }
          ...
          // oldKey is not in newChildrenKeys
          // but newKey is in oldChildrenKeys
          for (; oldIndex < oldChildrenLength; oldIndex++) {
            oldKey = oldChildrenKeys[oldIndex];
            if (oldKey === newKey && oldChildrenSearch[oldIndex] !== used) {
              break;
            }
            const to = newChildrenSearch.indexOf(oldKey);
            if (to === -1) {
          ...
          // newKey is not in oldChildrenKeys
          // but oldKey is in newChildrenKeys
          for (; newIndex < newChildrenLength; newIndex++) {
            newKey = newChildrenKeys[newIndex];
            if (newKey === oldKey) {
              break;
        ...
        else if ((oldInNew - newIndex) > (newInOld - oldIndex)) {
          for (; oldIndex < newInOld; oldIndex++) {
            oldKey = oldChildrenKeys[oldIndex];
            const to = newChildrenSearch.indexOf(oldKey);
            if (to === -1) {
              removed.push(oldIndex);
          ...
          for (; newIndex < oldInNew; newIndex++) {
            newKey = newChildrenKeys[newIndex];
            const from = oldChildrenSearch.indexOf(newKey);
            if (from !== -1) {
              oldChildrenSearch[from] = used;
    ...
    for (let i = 0, il = changes.length; i < il; i++) {
      const [index, change] = changes[i];
      const removed = change.removed;
      const nextIndex = (index + change.moved.length + removed.length);
      // Remove children
      for (oldIndex of removed) {
        nodeToRemove(oldChildren[oldIndex], transitions);
        checkDistinguishable(oldChildren, oldIndex, vnode, 'removed');
      }
      const added = change.added;
      const moved = change.moved;
      // Add children
      for (newIndex of added) {
        const newChild = newChildren[newIndex];
        if (newChild.domNode) {
          if (nextIndex < oldChildrenLength) {
            const oldChild = oldChildren[nextIndex];
            domNode.insertBefore(newChild.domNode, oldChild.domNode);
          }
          else {
          ...
          createDom(newChild, domNode, undefined, projectionOptions);
